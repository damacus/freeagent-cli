...
func TestNestedWorkflowPayloads(t *testing.T) {
	for _, tc := range []struct {
		...
		{[]string{"cis-settings", "update"}, "PUT", "/cis_settings", `{"cis_settings":{"subcontractor_details":{}}}`},
...
func TestNestedPayloadValidationPreventsRequests(t *testing.T) {
	for _, tc := range []struct{ args, body string }{
		...
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":[]}}`},
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":true}}`},
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":"registered"}}`},
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":1}}`},
		{"cis-settings update", `{"cis_settings":{"contractor_details":{}}}`},
		{"cis-settings update", `{"cis_settings":{"contractor_details":{"reporting_starts_on":""}}}`},
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":{"prior_deductions":{}}}}`},
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":{"prior_deductions":{"start_date":"2024-04-06"}}}}`},
		{"cis-settings update", `{"cis_settings":{"subcontractor_details":{"prior_deductions":{"initial_balance":"0"}}}}`},
